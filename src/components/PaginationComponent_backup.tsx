...
import "../routes/pagination.css"; // see pagination.css examples below
...
function PaginationComponent({ current_page_num, setCurrentPageNum, total_page_num }: Props) {
    ...
    const moveToPageForInputNum = () => {
        ...
        if (total_page_num !== undefined) {
            ...
            if (page_num_to_move < total_page_num) {
    ...
    return (
        <Flex height={12}>
            <Spacer />
            <HStack>
                <Pagination total={total_page_num} current={current_page_num} onPageChange={(page) => handlePageChange(page)} />
            </HStack>
            <Spacer />
            {/* <Box>
                <HStack mt={1}>
